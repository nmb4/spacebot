...
antigravity_key = "env:ANTIGRAVITY_API_KEY"
antigravity_project_id = "env:ANTIGRAVITY_PROJECT_ID" # required when using antigravity_key without OAuth
...
| `gemini_key` | string | None | Google Gemini API key (or `env:VAR_NAME`) |
| `ollama_key` | string | None | Ollama Cloud API key (or `env:VAR_NAME`) |
| `ollama_base_url` | string | None | Ollama base URL (local or cloud endpoint) |
| `opencode_zen_key` | string | None | OpenCode Zen API key (or `env:VAR_NAME`) |
| `nvidia_key` | string | None | NVIDIA NIM API key (or `env:VAR_NAME`) |
| `minimax_key` | string | None | MiniMax API key (or `env:VAR_NAME`) |
| `moonshot_key` | string | None | Moonshot API key (or `env:VAR_NAME`) |
| `kimi_coding_key` | string | None | Kimi Coding Plan API key (or `env:VAR_NAME`) |
| `zai_coding_plan_key` | string | None | Z.AI Coding Plan API key (or `env:VAR_NAME`) |
| `antigravity_key` | string | None | Antigravity access token (or `env:VAR_NAME`) |
| `antigravity_project_id` | string | None | Google Cloud `project_id` for Antigravity key-based auth |

When routing to Antigravity, use one of these IDs:
...
Spacebot automatically maps common legacy IDs to current Antigravity IDs (for example `claude-sonnet-4-5` -> `claude-sonnet-4-6`).
Spacebot also auto-tries compatible Antigravity client versions (`1.23.0`, `1.25.0`, `1.15.8`) unless `PI_AI_ANTIGRAVITY_VERSION` is set.
...
```toml
[llm.provider.<id>]
api_type = "anthropic"          # Required - one of: anthropic, gemini, openai_completions, openai_responses, antigravity
...
|-------|------|----------|-------------|
| `api_type` | string | Yes | API protocol type. One of: `anthropic` (Anthropic Messages API), `gemini` (Google Gemini OpenAI-compatible endpoint), `openai_completions` (OpenAI Chat Completions-compatible API), `openai_responses` (OpenAI Responses API-compatible), or `antigravity` (Cloud Code Assist SSE API) |
...
| `name` | string | No | Optional friendly name for the provider (displayed in logs and UI). For `antigravity`, this must be the Google Cloud `project_id` when not using OAuth credentials |
